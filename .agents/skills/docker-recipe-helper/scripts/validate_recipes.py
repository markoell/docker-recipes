...
def check_security(recipe_name, service_name, service_config, errors):
    # Check user
        ...
        if tmpfs:
            ...
            if isinstance(volumes, list):
                for v in volumes:
                    if isinstance(v, str):
                        parts = v.split(':')
                        if len(parts) >= 2 and parts[1] in ('/tmp', '/run', '/var/run'):  # nosec B108
                            has_tmp_mount = True
...
def check_traefik_labels(recipe_name, service_name, service_config, errors):
            ...
            if len(parts) >= 4:
                ...
                if parts[-1] == 'certresolver':
                    if v != 'dns_certresolver':
                        ...
                        errors += 1
    return is_traefik_enabled, errors

def check_network_isolation(recipe_name, service_name, service_config, is_traefik_enabled, errors):
...
def check_volumes(recipe_name, service_name, service_config, errors, warnings):
    volumes = service_config.get('volumes', [])
    if isinstance(volumes, list):
...
def validate_recipe(recipe_dir, compose_file):
    ...
    for service_name, service_config in services.items():
        ...
        errors = check_security(recipe_name, service_name, service_config, errors)
        
        # 4. Validate Traefik labels & Proxy network requirement
        is_traefik_enabled, errors = check_traefik_labels(recipe_name, service_name, service_config, errors)
        
        # 5. Check network isolation when Traefik is enabled
        errors = check_network_isolation(recipe_name, service_name, service_config, is_traefik_enabled, errors)
        
        # 6. Validate volumes
        errors, warnings = check_volumes(recipe_name, service_name, service_config, errors, warnings)
